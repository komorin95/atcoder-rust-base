...
fn main() {
    ...
    let m_modp1 = m.iter().map(|&x| ModP::<Mod9>::new(x)).collect_vec();
    let m_modp2 = m.iter().map(|&x| ModP::<Mod9_2>::new(x)).collect_vec();
    let l_modp1 = convolution(&m_modp1, &m_modp1);
    let l_modp2 = convolution(&m_modp2, &m_modp2);
    let l_modp1_temp = l_modp1.iter().map(|a| a.to_modm_u128()).collect_vec();
    let l_modp2_temp = l_modp2.iter().map(|a| a.to_modm_u128()).collect_vec();
    let l: Vec<ModM<u128>> = crt_slice(&l_modp1_temp, &l_modp2_temp).unwrap();
    let g = ModM::new(g, p);
    let mut result = 0;
    let mut g_pow_d = ModM::new(1, p);
    for d in 0..l.len() {
        result += g_pow_d.value() as u128 * l[d].value();
        g_pow_d = g_pow_d * g;
    }
    for &ai in &a {
        result -= (ModM::new(ai, p) * ModM::new(ai, p)).value() as u128;
    }
    result /= 2;
    println!("{}", result);
}
